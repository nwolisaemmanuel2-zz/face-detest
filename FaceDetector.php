...
class FaceDetector
{
  ...
  public function toFace()
    {

            define("WIDTH", 800);
            define("HEIGHT", 800);

            $dest_image = imagecreatetruecolor(800, 800);
            ...
            imagecopy($dest_image,$this->canvas, 0, 0, 0, 0, WIDTH, HEIGHT);
            imagecopy($dest_image, $a, $this->face['x']+0,$this->face['y']+0, 0, 0, 256, 256);
    ...
    {
        $color = imagecolorallocate($this->canvas, 255, 0, 0); //green
        $color2 = imagecolorallocate($this->canvas, 255, 0, 0); //red
        $color3 = imagecolorallocate($this->canvas, 255,0, 0); //red

        imagerectangle(
            $this->canvas,
            $this->face['x']+3,
            $this->face['y']+4,
            $this->face['x']+$this->face['w']+4,
            $this->face['y']+ $this->face['w']+4,
            $color3
        );
        ...
        imagerectangle(
            ...
            $this->face['x']+2,
            $this->face['y']+2,
            $this->face['x']+$this->face['w']+2,
            $this->face['y']+ $this->face['w']+2,
            $color2
        );


$white = imagecolorallocate($this->canvas, 255, 255, 255);
$grey = imagecolorallocate($this->canvas, 128, 128, 128);
$black = imagecolorallocate($this->canvas, 255, 0, 0);
imagefilledrectangle($this->canvas, 20, 0, 800, 80, $white);
 
$text = 'By Ankitkumar Singh'; 
$font = 'arial.ttf';
 
imagettftext($this->canvas, 30, 0, 30, 50, $black, $font, $text);
  header('Content-type: image/png');
        //imagepng($this->canvas,'ank.png');
...
imagefilledrectangle($canvas, 0, 5, 385, 35, $white);
...
// Add the text
imagettftext($canvas, 15, 0, 10, 30, $black, $font, $text);

        imagejpeg($canvas, $outFileName);
        imagejpeg($canvas,'crop.jpg');
